...
def init_db():
    ...
    print("Banco inicializado em:", app.config["SQLALCHEMY_DATABASE_URI"])
...
def export_bookings():
    si = io.StringIO()
    writer = csv.writer(si)
    writer.writerow(["id","guest_name","guest_phone","check_in","check_out","status","price_total","note","created_at"])
    for b in Booking.query.order_by(Booking.id.asc()).all():
        writer.writerow([
            ...
            b.status,
            f"{b.price_total:.2f}" if b.price_total is not None else "",
            b.note or "",
